    ...
    for s in range(step):
        for i in range(n):
            for j in range(dataNum):
                ...
                gamaArray[i][j] = k[i] * normPdf(dataArray[j], mu[i],
                                                 sigma[i]) / float(Sum)
        # 更新 mu
        for i in range(n):
        ...
        for i in range(n):
            sigma[i] = np.sqrt(
                np.sum(gamaArray[i] *
                       (dataArray - mu[i])**2) / np.sum(gamaArray[i]))
        # 更新系数k
        for i in range(n):
...
if __name__ == '__main__':
    ...
    # dataNum = 5000
    # 产生数据
    # TOdo dataArray = generateData(k, mu, sigma, dataNum)
    dataframe = pd.read_excel("fengsu.xlsx")
    dataArray = np.zeros(144, dtype=np.float32)
    for i in dataframe.index:
        dataArray[i] = dataframe['风速'][i]
    print(dataArray)
